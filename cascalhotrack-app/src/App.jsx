...
function TelaGestor({viagens,setViagens,caminhoes,setCaminhoes,destinos,setDestinos,tabela,setTabela,apiKey,setApiKey}) {
  ...
  const addCam=()=>{if(!nPlaca||!nMot||!nFret||!nVol){showToast("Preencha todos os campos!","error");return;}setCaminhoes(p=>[...p,{id:p.length+1,placa:nPlaca.toUpperCase(),motorista:nMot,freteiro:nFret,volumeM3:parseFloat(nVol),whatsapp:nWa.replace(/\D/g,"")}]);setNPlaca("");setNMot("");setNFret("");setNVol("");setNWa("");setShowAddCam(false);showToast("Caminhão cadastrado!");};
  const addDest=()=>{if(!nDNome||!nDKm){showToast("Preencha todos os campos!","error");return;}const m=parseInt(nDKm)||0;setDestinos(p=>[...p,{id:p.length+1,nome:nDNome,distanciaM:m}]);setNDNome("");setNDKm("");setShowAddDest(false);showToast("Destino cadastrado!");};
          ...
          <div>
            ...
            {destinos.map(d=>(
              <Card key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><div style={{fontWeight:700}}>{d.nome}</div><div style={{fontSize:12,color:"#9090a0"}}>{d.distanciaM}m ({(d.distanciaM/1000).toFixed(1)}km)</div></div>
            ...
            {caminhoes.map(c=>(
              <Card key={c.id} style={{display:"flex",alignItems:"center",gap:10}}>
                <img src={qrUrl(`CASCALHOTRACK:${c.placa}:${c.id}`)} alt={c.placa} style={{width:48,height:48,borderRadius:6,border:"2px solid #c4600a44"}}/>
                <div>
                  <div style={{fontWeight:700}}>{c.placa}</div>
